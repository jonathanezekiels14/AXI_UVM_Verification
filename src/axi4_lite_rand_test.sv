class axi4_lite_rand_test extends uvm_test;
	`uvm_component_utils(axi4_lite_rand_test)

	axi4_lite_environment env;
	axi4_lite_config cfg;

	function new(string name = "axi4_lite_rand_test", uvm_component parent = null);
	...
	virtual function void end_of_elaboration_phase(uvm_phase phase);
		...
		`uvm_info("RAND_TEST", "Printing UVM Topology:", UVM_NONE)
		uvm_top.print_topology();
	endfunction

	virtual task run_phase(uvm_phase phase);
		axi4_lite_rand_vseq rvseq;
		
		phase.phase_done.set_drain_time(this, 500ns);
		phase.raise_objection(this);
		
		rvseq = axi4_lite_rand_vseq::type_id::create("cvseq");
		rvseq.start(env.vsqr);
